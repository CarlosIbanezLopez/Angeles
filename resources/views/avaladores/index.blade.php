<x-app-layout>
    <x-slot name="header">
        <div class="flex align-items-center">
            ...
            @can('admin.avaladores.create')
                <a href="{{ route('avaladores.create') }}" class="ml-4 text-sm flex justify-center items-center max-sm:w-3/4"
                    title="{{ __('Agregar un avalador') }}">
                                    ...
                                    @foreach ($avaladores as $avalador)
                                        <tr class="bg-white border-b">
                                            ...
                                            <td class="py-2 px-2 flex justify-between">
                                                @can('admin.avaladores.edit')
                                                    <a href="{{ route('avaladores.edit', $avalador) }}">
                                                        <img src="{{ asset('img/edit-button.png') }}" alt="Editar avalador"
                                                            class="h-auto w-6 md:w-6" title="{{ __('Edit') }}">
                                                    </a>
                                                @endcan
                                                @can('admin.avaladores.delete')
                                                    <form action="{{ route('avaladores.destroy', $avalador) }}"
                                                        method="POST" class="inline">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit">
                                                            <img src="{{ asset('img/delete.png') }}"
                                                                alt="Eliminar avalador" class="h-auto w-6 md:w-6 tbl-hapus"
                                                                title="{{ __('Delete') }}">
                                                        </button>
                                                    </form>
                                                @endcan
