        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    {{-- form edit --}}
                    <form action="{{ route('notasalidas.update', $notasalida) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="mb-6">
                        ...
                        <div class="mb-6">
                            <label for="motivo" class="block mb-2 text-sm font-medium text-gray-900">
                                {{ __('Motivo de salida de los muebles') }}
                            </label>
                            <input type="text" id="motivo" name="motivo"
                                value="{{ old('motivo', $notasalida->motivo) }}"
                        ...
                        <div class="from-group">
                            <label for="inventario_id" class="block mb-2 text-sm font-medium text-gray-900">
                                {{ __('Muebles del inventario retirados') }}
                            </label>
                            <select id="inventario_id" name="inventario_id[]"
                                ...
                                @foreach ($inventarios as $inventario)
                                    <option value="{{ $inventario->id }}"
                                        {{ in_array($inventario->id, old('inventarios', $notasalida->inventarios->pluck('id')->toArray())) ? 'selected' : '' }}>
