    ...
    <x-slot name="header">
        <div class="flex align-items-center">
            ...
            @can('admin.residentes.create')
                <a href="{{ route('residentes.create') }}" class="ml-4 text-sm flex justify-center items-center max-sm:w-3/4"
                    title="{{ __('Agregar un residente') }}">
                                                ...
                                                @can('admin.residentes.edit')
                                                    <a href="{{ route('residentes.edit', $residente) }}">
                                                        <img src="{{ asset('img/edit-button.png') }}"
                                                            alt="Editar residente" class="h-auto w-6 md:w-6"
                                                            title="{{ __('Edit') }}">
                                                    </a>
                                                @endcan
                                                @can('admin.residentes.delete')
                                                    <form action="{{ route('residentes.destroy', $residente) }}"
                                                        method="POST" class="inline">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit">
                                                            <img src="{{ asset('img/delete.png') }}"
                                                                alt="Eliminar residente" class="h-auto w-6 md:w-6 tbl-hapus"
                                                                title="{{ __('Delete') }}">
                                                        </button>
                                                    </form>
                                                @endcan
