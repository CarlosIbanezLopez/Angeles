          ...
          <form action="{{ route('empleados.update', $empleado) }}" method="POST">
            ...
            <div class="mb-6">
              ...
              <select id="sexo" name="sexo" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                <option value="M" @selected($empleado->sexo == 'M')>Masculino</option>
                <option value="F" @selected($empleado->sexo == 'F')>Femenino</option>
              </select>
              @error('sexo')
              <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
              @enderror
            ...
            <div class="mb-6">
              ...
              <input type="number" id="telefono" name="telefono" value="{{ old('telefono', $empleado->telefono) }}"
            ...
            <div class="mb-6">
              ...
              <select id="id_edif" name="id_edif" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                @foreach ($edificios as $edificio)
                  <option value="{{ $edificio->id }}" @selected($edificio->id == $empleado->edificio->id)>{{ $edificio->nombre.' - '.$edificio->getCiudad() }}</option>
                @endforeach
              </select>
              @error('id_edif')
              <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
